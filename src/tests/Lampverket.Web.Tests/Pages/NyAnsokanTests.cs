...
{
    private static BunitContext CreateCtx(IAnsokanService? handlaggare = null)
    {
        var ctx = new BunitContext();
        ctx.Services.AddSingleton<IAnsokanService>(
        ...
        ctx.Services.AddSingleton<IOptions<HomeAssistantOptions>>(Options.Create(new HomeAssistantOptions
        {
            Devices =
            [
                new DeviceMapEntry { Friendly = "Testlampa", Area = "Rum", EntityId = "light.test" }
            ]
        }));
        return ctx;
    }
    ...
    public void NyAnsokan_Media_EmptySokterm_ShowsError()
    {
        using var ctx = CreateCtx();
        var cut = ctx.Render<NyAnsokan>();
        cut.Find("select[id='arendetyp']").Change(nameof(Arendetyp.Media));
        // leave sökterm blank
    ...
    public void NyAnsokan_Ljusstyrka_ShowsRangeInput()
    {
        ...
        dropdown.Change(nameof(Arendetyp.Ljusstyrka));
    ...
    public void NyAnsokan_Tandning_NoRangeInput()
    {
        ...
        var cut = ctx.Render<NyAnsokan>();
        var dropdown = cut.Find("select[id='arendetyp']");
        dropdown.Change(nameof(Arendetyp.Tanding));
    ...
    public void NyAnsokan_Media_ShowsTextInput()
    {
        ...
        var cut = ctx.Render<NyAnsokan>();
        var dropdown = cut.Find("select[id='arendetyp']");
        dropdown.Change(nameof(Arendetyp.Media));
    ...
    public async Task NyAnsokan_ValidSubmit_CallsHandlaggareService()
    {
        ...
        cut.Find("input[id='personnummer']").Change("19900101-1234");
        cut.Find("select[id='arendetyp']").Change(nameof(Arendetyp.Tanding));
        ...
        await Task.Delay(50);
        Assert.True(fake.WasCalled, "IAnsokanService.RegisterAnsokanAsync should have been called");
    }
}

// ── Fakes ────────────────────────────────────────────────────────────────────

file sealed class FakeHandlaggareService : IAnsokanService
{
